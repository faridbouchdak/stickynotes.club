...
Private-board collaboration synchronises accepted changes without requiring a page refresh and autosaves editable sticky-note text. The interface makes saving, success and failure visible, preserves unsaved text through connection or server failures and provides a safe retry path. A simultaneous text change never silently overwrites local work: the local version remains available while the user resolves the conflict. Hearts, votes and position changes use the server’s accepted state as authoritative.
